...
### boj 16500번
    ...
    문자열과 단어목록이 주어졌을때 단어목록을 통해 문자열을 만들수있는가?
    단어목록을 뒤지면서 replace로 문자열을 지우면서 해봤는데 안됬다.
    반례가 있었음.
    abac
    4
    a
    ...
    그러면 이차원배열로 만든뒤 모든 경우를 찾아보면? 경우의수가 너무 커서 포기
    결국 포기하고 블로그 탐색. https://velog.io/@cchloe2311/알고리즘-백준-16500.-문자열판별
    문자열을 1,2,3,4 ... 이렇게 뒤로 하나씩 늘리면서 모든 단어목록과 비교한다.
    그때 만약 일치하고 남은 값도 이전에 true로 확인된 값이면 해당 문자열은 만들수있는 문자열이므로 true로한다.
    dp의 발상은 참... 어렵다
